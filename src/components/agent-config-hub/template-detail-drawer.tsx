    ...
    <>
      ...
      <aside className="absolute bottom-0 right-0 top-0 z-10 w-96 border-l border-border bg-background shadow-xl">
                ...
                >
                  {editingTag ? (
                    <span className="inline-flex items-center gap-1">
                      ...
                      <button
                        onClick={async () => {
                          await updateTag(template.id, tagValue);
                          setEditingTag(false);
                        }}
                    ...
                    <span
                      ...
                      onDoubleClick={(e) => {
                        e.stopPropagation();
                        setTagValue(template.tag || "default");
                        setEditingTag(true);
                      }}
              ...
              <div className="relative group/code">
                ...
                <pre className="max-h-[360px] overflow-y-auto bg-muted/40 dark:bg-black/40 p-4 text-[12px] leading-relaxed text-foreground dark:text-[#D4D4D4] font-mono scrollbar-thin scrollbar-thumb-foreground/10 hover:scrollbar-thumb-foreground/20">
                  {isLoading ? (
                    ...
                    <span className="text-destructive/90">{errorMsg}</span>
                  ) : (
                    content || "(empty file)"
                  )}
                </pre>
        ...
        {confirmDelete && (
          <div
            role="dialog"
            aria-label="Delete agent config template"
            className="absolute inset-x-4 bottom-4 rounded-lg border border-border bg-background p-3 shadow-lg"
          >
            <p className="text-sm text-muted-foreground">
              Delete only removes this template from
              ~/.harnesskit/agent-configs. Project files stay untouched.
            </p>
      ...
      {showEditor && (
        <div className="fixed inset-0 z-20">
          ...
          <div
            role="dialog"
            aria-label="Edit agent config content"
            className="absolute inset-x-6 top-1/2 z-10 flex -translate-y-1/2 justify-center"
          >
            <div className="w-[min(900px,calc(100vw-48px))] rounded-2xl border border-border/50 bg-card shadow-2xl overflow-hidden">
              <div className="flex items-center justify-between border-b border-border/50 px-5 py-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Edit and save this agent config template.
                  </p>
                </div>
                <button
                  onClick={() => setShowEditor(false)}
