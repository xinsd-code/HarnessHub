...
<p align="center">
  <img src="media/agents-animation.gif" alt="HarnessHub Agents Animation" width="800" />
</p>
...
### 📦 Kits 与 HarnessHub 组合包

将 **Skill**、**MCP Server** 与 **CLI** 打包成带版本、可追溯的 **Kit** 扩展套件，或进一步组合 Kit 与 **Agent 配置模板** 成为一个 **HarnessHub 组合包**，一键同步到任意项目与 Agent。

- **扩展套件（Kit）** —— 从已安装扩展或 Local Hub 中挑选资产，组合成一个 Kit。Sync 时如果资产尚未备份到 Local Hub，系统会自动推送后再安装。
- **HarnessHub 组合包** —— 将多个 Kit 和 Agent 配置模板打包在一起。一次同步即可完成扩展安装 + 配置文件写入，适合团队标准化开发环境。
...
- **CLI**：Rust 二进制，作为独立命令行工具（`hk`）管理扩展
