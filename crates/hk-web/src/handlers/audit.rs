...
pub async fn list_audit_results(State(state): State<WebState>) -> Result<Vec<AuditResult>> {
    blocking(move || {
...
pub async fn run_audit(State(state): State<WebState>) -> Result<Vec<AuditResult>> {
    blocking(move || {
        let store = state.store.lock();
