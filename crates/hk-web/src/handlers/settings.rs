...
pub async fn get_dashboard_stats(State(state): State<WebState>) -> Result<DashboardStats> {
    blocking(move || {
        ...
        Ok(DashboardStats {
            ...
            skill_count: exts
                .iter()
                .filter(|e| e.kind == hk_core::models::ExtensionKind::Skill)
                .count(),
            mcp_count: exts
                .iter()
                .filter(|e| e.kind == hk_core::models::ExtensionKind::Mcp)
                .count(),
            plugin_count: exts
                .iter()
                .filter(|e| e.kind == hk_core::models::ExtensionKind::Plugin)
                .count(),
            hook_count: exts
                .iter()
                .filter(|e| e.kind == hk_core::models::ExtensionKind::Hook)
                .count(),
            cli_count: exts
                .iter()
                .filter(|e| e.kind == hk_core::models::ExtensionKind::Cli)
                .count(),
...
pub async fn get_all_tags(State(state): State<WebState>) -> Result<Vec<String>> {
    blocking(move || {
        let store = state.store.lock();
        store.get_all_tags()
    })
...
pub async fn get_all_packs(State(state): State<WebState>) -> Result<Vec<String>> {
    blocking(move || {
        let store = state.store.lock();
        store.get_all_packs()
    })
...
pub async fn toggle_by_pack(
    ...
    blocking(move || {
        ...
        for id in &ids {
            manager::toggle_extension_with_adapters(&store, &state.adapters, id, params.enabled)?;
        }
        Ok(ids)
    })
    .await
...
pub async fn read_config_file_preview(
    ...
    blocking(move || {
        ...
        if !super::is_path_allowed(&canonical, &store) {
            return Err(hk_core::HkError::PermissionDenied(
