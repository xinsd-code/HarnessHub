...
-> Result<Json<Vec<agent_config_templates::AgentConfigTemplate>>, ApiError> {
    blocking(|| agent_config_templates::list_templates(&agent_config_templates::default_hub_dir()))
        .await
}

pub async fn get_agent_config_template_content(
    Json(req): Json<ContentReq>,
) -> Result<Json<String>, ApiError> {
    blocking(move || {
        agent_config_templates::read_template_content(
...
pub async fn import_agent_config_template(
    Json(req): Json<ImportReq>,
) -> Result<Json<agent_config_templates::AgentConfigTemplate>, ApiError> {
    blocking(move || {
...
pub async fn update_agent_config_template_tag(
...
pub async fn update_agent_config_template_content(
...
pub async fn delete_agent_config_template(
    Json(req): Json<ContentReq>,
) -> Result<Json<()>, ApiError> {
    blocking(move || {
        agent_config_templates::delete_template(&agent_config_templates::default_hub_dir(), &req.id)
    })
    .await
}
...
pub async fn sync_agent_config_template_to_project(
    ...
    blocking(move || {
        ...
        let target_relpath = req.rel_path.filter(|p| !p.trim().is_empty()).or_else(|| {
            adapters
                .iter()
